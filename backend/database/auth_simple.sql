...
-- Create indexes for enhanced search and filtering (ignore errors if they exist)
CREATE INDEX idx_users_approval_status ON users(approval_status);
CREATE INDEX idx_users_is_verified ON users(is_verified);
CREATE INDEX idx_users_profile_completion_step ON users(profile_completion_step);
CREATE INDEX idx_users_created_at ON users(created_at);
CREATE INDEX idx_users_role_status ON users(user_type, approval_status);